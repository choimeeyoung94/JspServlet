 ...
 * 
 * 1. 데이터베이스와의 연결, 데이터 조회/삽입/수정/삭제(CRUD) 등 데이터 접근 작업을 수행하는 객체
 * 2. 비즈니스 로직과 데이터베이스 로직을 분리하여 코드 구조화, 쉬운 유지보수, 코드 재사용성등을 얻을 수 있다
 * 3. 인터페이스를 통해 DAO 객체를 생성하면 구현체를 자유롭게 변경할 수 있다
 * 
 * */
public class BoardDAO {
  ...
  // 조회 (단일 항목)
  public BoardDTO getBoardBy(int bid) {
    BoardDTO board = null;
   try {
     con = DBUtils.getConnection();
